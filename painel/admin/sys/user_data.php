...
<?
	# Consulta SQL
	$sql = "SELECT
			user.*, z_ministerios_categoria.*, DATE_FORMAT(data,'%d/%m/%Y') as data1
	
	FROM 	user
			LEFT JOIN z_ministerios_categoria ON (z_ministerios_categoria.id_categoria = user.ministerio)
	
	WHERE
	user.nome LIKE '%$nome%'
	AND user.sobrenome LIKE '%$sobrenome%'
	AND user.cpf LIKE '%$cpf%'
	AND user.talentos LIKE '%$talentos%'
	AND user.data LIKE '%$data%'
	
	ORDER BY user.data DESC
...
<?
	...
	# Listando
	echo adminLista($campos,$dados,array('excluir','editar','status','visualizar'),$Config,true);
