<? 
   ...
   $num_busca = mysql_num_rows($exe_busca);
	...
	if ($_GET['faz']=="dados") {
		...
		# Testes
		$Erros='';
		if (strlen($nome) < 0) $Erros .= "O nome é obrigatório";
		if ((mysql_num_rows($exe_busca) > 0) && (!($id>0)) ) $Erros .= "Este login já está sendo usado, tente outro";
		if (  (! validaTipoArquivo($_FILES['imagem']['name'],1)) && (!($id>0)) ) $Erros .= "Tipo de arquivo não aceito! Envie JPG, GIF ou PNG";
