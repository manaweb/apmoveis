<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Testes
		$Erros='';
		if (strlen($titulo) < 2) $Erros .= "Digite um título<br>";
		if (  (! validaTipoArquivo($_FILES['imagem']['name'],1)) && (!($id_agenda>0)) ) $Erros .= "<br>Tipo de arquivo não aceito! Envie JPG, GIF ou PNG";
		...
		# Dados
		$dados = array( 'id_agenda'=>$id_agenda, 'titulo'=>$titulo, 'data'=>dataDMY_YMD($data), 'descricao'=>$descricao);
		...
		if ($$Config['id']>0) {
			...
			$dados['id_agenda']=$_SESSION['Admin']['id_agenda'];
			db_executa($Config['tabela'],$dados);
			
			# Cadastrar novo endereço
			$dados_end = array('id_agenda'=>$id_agenda);
