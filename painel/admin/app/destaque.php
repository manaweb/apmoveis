<? 
	...
	if ($_GET['faz']=="dados") {
		...
		# Testes
		$Erros='';
		if (strlen($titulo) < 2) $Erros .= "informe um título para a postagem";
		if (  (! validaTipoArquivo($_FILES['imagem']['name'],1)) && (!($id_destaque>0)) ) $Erros .= "<br>Tipo de arquivo não aceito! Envie JPG, GIF ou PNG";
		...
		# Dados
		$dados = array( 'id_destaque'=>$id_destaque, 'titulo'=>$titulo, 'categoria'=>$categoria, 'descricao'=>$descricao, 'destino'=>$destino);
		...
		if ($$Config['id']>0) {
			...
			$dados['id_destaque']=$_SESSION['Admin']['id_destaque'];
			db_executa($Config['tabela'],$dados);
			
			# Cadastrar novo endereço
			$dados_end = array('id_destaque'=>$id_destaque);
