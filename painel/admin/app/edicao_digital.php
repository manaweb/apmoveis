<? 
	...
	$Config = array(
		'arquivo'=>'edicao_digital',
		'tabela'=>'tbedicao_digital',
		'titulo'=>'titulo',
		'id'=>'id_edicao',
		'urlfixo'=>'', 
		'pasta'=>'edicao_digital',
		'imagem'=>array(
			'x'=>400, 'y'=>400, 'corte'=>0, 
			'mini'=>array(
				'x'=>100, 'y'=>100, 'corte'=>1
	...
	if ($_GET['faz']=="dados") {
		...
		if (  (! validaTipoArquivo($_FILES['imagem']['name'],1)) && (!($id_edicao>0)) ) $Erros .= "<br>Tipo de arquivo não aceito! Envie JPG, GIF ou PNG";
		...
		if ($arquivo_metodo==1) {
			...
			if (!empty($arquivo_ftp)) {
				$dados['arquivo'] = nomeArquivo($arquivo_ftp, "../../arquivos/".$Config['pasta']."/");
				@copy('../../arquivos/ftp/'.$arquivo_ftp, "../../arquivos/".$Config['pasta']."/".$dados['arquivo'] );
			}
