...
function PaginarGaleria($pgatual,$pgtotal) {
	...
	if ($_GET['faz'] == 'foto') {
		list($codigo) = db_lista(db_consulta("SELECT codigo FROM tbartistas WHERE id_artistas=".(int)$_GET['id_artistas']." LIMIT 1;"));
		list($imagem,$legenda, $id_foto) = db_lista(db_consulta("SELECT imagem, legenda, id_foto FROM tbartistas_fotos WHERE id_artistas=".(int)$_GET['id_artistas']." AND flag_status=1 ORDER BY posicao ASC, id_foto ASC LIMIT ".(int)( $_GET['pos']-1 ).",1"));
		db_consulta("UPDATE tbartistas_fotos SET contador=contador+1 WHERE id_foto=".$id_foto." LIMIT 1;");
		?>
        	<a href="javascript:galeriaProxFoto()"><img src="../arquivos/artistas/<?=$codigo;?>/fotos/<?=$imagem;?>" class="foto" /></a>
	...
	if ($_GET['faz'] == 'mini') {
        ...
        $minis = db_consulta("SELECT imagem, legenda FROM tbartistas_fotos WHERE id_artistas=".(int)$_GET['id_artistas']." AND flag_status=1 ORDER BY posicao ASC, id_foto ASC LIMIT ".$inicio.",".(int)$_GET['pp']);
		...
		while (list($imagem,$legenda) = db_lista($minis)) { $i++; $cont++;
			...
			?>
        		<td><a href="javascript:void(null);" onclick="galeriaCarregaFoto(<?=$i;?>);"><img id="miniatura<?=$i;?>" class="normal" onmouseout="if (this.className!='ativo') this.className='normal';" onmouseover="if (this.className!='ativo') this.className='selecionado';" src="../arquivos/artistas/<?=$codigo;?>/miniaturas/<?=$imagem;?>" /></a></td>
            <?
	...
	if ($_GET['faz'] == 'comentarios') {
        $i=0;
        $SQL = "SELECT tbartistas_comentarios.*,tbusuarios.*, DATE_FORMAT(datahora,'%H:%i do dia %d/%m/%Y') as data1 FROM tbartistas_comentarios INNER JOIN tbusuarios ON (tbusuarios.id_usuario = tbartistas_comentarios.id_usuario) WHERE id_artistas=".$_GET['id_artistas']." AND flag_status=1 ORDER BY datahora DESC ";
		...
		if (db_linhas($consulta)>0) {
			...
			echo utf8_encode('<div class="paginacao"><b>Páginas: </b>'.PaginarComentarios($_GET['pag'],$Paginacao->totalPaginas(),$_GET['id_artistas'],'galComentarios').'</div>');
