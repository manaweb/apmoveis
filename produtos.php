          ...
          <?php
			...
			while ($dados = mysql_fetch_assoc($q)) {
				...
				while ($dados2 = mysql_fetch_assoc($qq))
					$final .= '<hr class="separador-menu"><li><a href="produtos.php?id='.$dados2['id_subcategoria'].'">'.utf8_encode($dados2['subcategoria']).'</a></li>';
  ...
  <script>
      jQuery(function ($) {
          ...
          function carregaProdutos(){
          	...
          	pesquisa = '<?=$pesquisa?>';
			$.ajax({
				url: "retornoProdutos.php?lastId="+ $(".produtinho:last").attr('id')+'&' + catid + id + pesquisa,
				type: 'GET',
				dataType: 'html',
				beforeSend: function() {
